...
struct MANGOS_DLL_DECL npc_horsemans_headAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        Creature* pHorseman = m_creature->GetMap()->GetCreature(m_pInstance->GetData64(NPC_HEADLESS_HORSEMAN));
        if (!pHorseman)
            return;
...
struct MANGOS_DLL_DECL boss_headless_horsemanAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        if (m_pInstance)
            m_pInstance->SetData64(NPC_HEADLESS_HORSEMAN,m_creature->GetGUID());
    }
